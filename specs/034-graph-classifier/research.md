...
- **Rationale:** Unifies the parallel type hierarchies while maintaining the exact public API of `GraphLens`. The two categories will be `GNode` (when `testNode` holds) and `GOther ()` (when it doesn't).
- **Alternatives considered:** Keeping `GraphLens` separate (creates duplicate work for graph queries).

## Default Classification Logic
- **Decision:** `classifyByShape` is extracted from the `GraphValue` typeclass and packaged into `canonicalClassifier :: GraphClassifier () Subject`. Crucially, Walk classification is upgraded: walks must be validated as a contiguous sequence of relationships that "chain" by sharing nodes between consecutive relationships (ignoring relationship direction).
