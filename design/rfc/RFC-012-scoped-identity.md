...
A `Subject`'s bare local identity becomes a stable, clash-free identity when patterns are ingested into a shared store. Identity is **`(scope, local)`**: an element's resolved identity is its Frame's identity plus a local discriminator — one step, not a chain, because RFC-001's Frame registry already assigns every member, named or anonymous, a flat, Frame-scoped address at admission. Named elements resolve to `(frame_scope, name)`; anonymous elements resolve to `(frame_scope, ordinal)` using that same flat `Positional` ordinal — not an owner-relative discriminator, and not exclusive to any one container. A Frame's scope is declared by a reserved header key **`namespace`** and, when absent, assigned by the store on first ingest. Truly anonymous elements remain permitted everywhere in text and in memory; they are constrained in exactly one place — they may not be the *target* of a by-identity reference (a cross-Frame Span endpoint or a stored foreign key) until promoted, because a reference target needs an author-chosen, domain-meaningful identity, which a bare ordinal is not. Promotion is anchored to a Frame. The design draws its two load-bearing mechanisms from prior art that maps almost one-to-one onto the problem: RDF's document-scoped blank nodes and skolemization, and Datomic's per-transaction tempids resolved to durable ids at the boundary.

## Motivation

RFC-011 (Codec) can persist a `Pattern v` into a store, but its Open Question 3 flagged a prerequisite it could not resolve within its own scope: **scoped identity namespaces.** A `.gram` file carries its own id-space — human-chosen symbols (`alice`) and anonymous elements the parser leaves without any identity of their own (`Symbol ""`, per `Gram.fromGram`), meaningful only once a Frame admits them and assigns each a flat ordinal within that Frame. The moment two files are ingested into one store, those id-spaces collide, and RFC-011's upsert (which keys on identity) has no principled way to know whether two `alice`s are the same entity or two different ones. RFC-011 committed the local rule — identity is owned by `Subject.identity`, the store never mints an identity of record — but explicitly deferred the cross-cutting question of how id-spaces are scoped, qualified, or grounded on ingest to a future RFC. This is that RFC.
...
2. **Anonymous identity that travels.** An anonymous element carries no ordinal of its own until admitted into a Frame (RFC-001), which assigns it a flat, Frame-scoped ordinal. When such an element must be referenced from outside its Frame (a cross-Frame Span endpoint, a stored row), it needs a stable, clash-free identity — i.e., promotion to a name.
...
Every element's resolved identity is a pair: a **scope** and a **local discriminator**. The scope is the identity of the element's **Frame** (RFC-001) — resolution is one step, not a chain. RFC-001's Frame registry already assigns every member a `LocalAddress` at admission — `Named LocalIdentity` or `Positional ElementOrdinal` — and that address carries no reference to any containing member: containment is recorded by whichever registry entry lists the address among its own elements, never by the address itself (RFC-001 §Identity and addresses). So an element's resolved identity is directly `(frame_scope, local_address)`; there is no ancestor to walk to get there. This is a deliberate correction from an earlier draft of RFC-001 itself, which scoped an anonymous member's address to its (single) containing member — rejected there because it prevented one anonymous member from being shared by more than one container, exactly the case RFC-001's Worked Exercise now exercises (a shared `Note` attached to two members).

Two populations sit at this one level:
...
- **Anonymous elements resolve to `(frame_scope, ordinal)`** using the same flat, Frame-scoped ordinal RFC-001 assigns at admission — not an owner-relative discriminator, and not exclusive to any one container. This is *not* the ER weak-entity / identifying-relationship pattern (a weak entity has exactly one identifying owner by construction; a flat ordinal has none — nothing about the ordinal names or requires a container at all). The ordinal exists because a Pattern's elements are an ordered sequence (RFC-002) admitted in a fixed order, and because RFC-003 fixes that two anonymous patterns are always distinct even when structurally identical — an ordinal preserves that distinctness where a content hash would collapse it.

Because every anonymous element's ordinal is assigned once, at admission, and is never recomputed from tree position, editing an unrelated part of a Frame cannot perturb it: inserting a new anonymous member does not shift any existing member's ordinal, named or positional. This fully resolves what was Open Question 1 in an earlier draft of this RFC (ordinal fragility under reordering) rather than merely bounding its blast radius — see RFC-001 §Reconciliation ("positional addresses are as stable as named ones once assigned").

### The Frame grounds resolution — and is required for storage

"Storage requires a Frame" and "promotion is anchored to a Frame" are the same statement. A bare local address (a name, or an ordinal) means nothing until grounded against an absolute scope; the Frame is that ground. Outside any Frame — in memory, or in gram text before admission — an element's identity is simply its position in whatever structure holds it, sufficient for structural navigation but not for reference from outside that structure. Admission into a Frame (RFC-001) is what grounds it: every member, named or positional, receives a `LocalAddress` in that Frame's registry, and `(frame_scope, local_address)` is its resolved identity from that point on.

Resolution is therefore a total function of Frame admission, not a walk over the in-memory `Pattern`: RFC-001's Frame registry assigns every admitted member's address directly, with no ancestor lookup involved. Promotion (below) is a distinct, later step — minting a `LocalIdentity` for a `Positional` address that must become a reference target — and that minted identity is an output of promotion, never an input to ordinary resolution. A codec chooses its representation and is faithful iff round-tripping preserves every *named* resolved identity — it does not get to redefine `(frame_scope, name)`. A `Positional` address's ordinal is not held to that same bar: per RFC-001 §Deferred Pattern conversion, round-trip correctness is topological isomorphism of the Frame registry, not reproduction of any particular ordinal, so a faithful codec may reassign one on re-admission as long as the containment shape and every named identity survive. This is the general answer to "which codecs must this account for": all of them, by reference to the one resolution defined here, not by enumeration.
...
- **Positional identity** — the element *is* its location ("the second child of `alice`"). The containing structure supplies identity; no name is needed. This suffices for gram text, and for navigating a single in-memory tree. This tree-relative sense of "position" is the *pre-Frame* one; once a Frame admits the element, RFC-001 assigns it a flat, Frame-wide ordinal at admission — unrelated to nesting depth or which member contains it — so "position" changes mechanics at the Frame boundary even though the category (unnamed, location-based identity) carries through.
...
Because promotion is anchored to a Frame, the feared impossibility — "an anonymous element that is a cross-Frame Span endpoint" — cannot arise, but not because admission into a Frame promotes an element by itself: RFC-001 gives every admitted member a stable `LocalAddress` (named or positional) without changing whether it is nameable, and an unpromoted `Positional` address is explicitly rejected as a Span pair endpoint (RFC-001's Worked Exercise, step 8). Admission is the *precondition* for promotion — a Span endpoint is necessarily inside a Frame, so a promotion operation always has a Frame to ground the resulting `LocalIdentity` in — not the act of promotion itself, which remains a distinct, separately designed operation (RFC-001 §Defining and reference occurrences).
...
| Framed anonymous | `(frame_scope, ordinal)` — stable once assigned | No — not a valid by-identity reference target |
| Named | `(frame_scope, name)` | Yes |

Framing makes an element's identity *stable and addressable within the Frame*; only naming makes it *referenceable* by another Frame, a Span pair, or a stored foreign key. The middle tier is not a stability problem — a framed-anonymous ordinal is exactly as stable as a name once admitted (see above) — it is a different requirement: a by-identity reference target must be author-chosen and domain-meaningful, and a bare ordinal categorically is not one, no matter how stable it is. This yields a single best-practice sentence, for a different reason than before: **name an element before referencing it across a Frame boundary.** Framing alone establishes a well-defined, stable address; only a name establishes one safe to reference.
...
**Skolemization at promotion.** Promotion mints a `LocalIdentity` for an already-admitted member's `Positional` address, replacing it with a `Named` one in the Frame registry — there is no path to resolve, since the member's ordinal is already flat and already unique within the Frame; promotion only needs to choose a name for it. This is RDF's spec-sanctioned skolemization ([Concepts §3.5](https://www.w3.org/TR/rdf11-concepts/)) performed deterministically at the Frame boundary. After promotion it is an ordinary named Frame member; the prior ordinal is not used in resolution again (Datomic's tempid→permanent-id move).
...
5. A contained-only anonymous element round-trips through a Frame under topological isomorphism (RFC-001 §Deferred Pattern conversion): the containment shape and its named identities survive, but its ordinal is not itself part of what round-tripping must preserve and a faithful codec may reassign it. An anonymous element made a cross-Frame Span endpoint is **promoted or rejected**, never silently keyed on `Symbol ""`.
...
1. **Identity model.** Introduce a resolved-identity representation distinguishing named (`(scope, name)`) from anonymous (`(scope, ordinal)`), grounded directly at a Frame's `LocalAddress` with no ancestor resolution required. Decide the anonymity representation (Open Question 3).
...
1. **Ordinal fragility within a named parent. — Resolved by RFC-001's flat-ordinal model.** An earlier draft of this RFC scoped an anonymous element's ordinal to its nearest named parent, so inserting a sibling could shift other siblings' ordinals and change their promoted ids. RFC-001's Frame registry (§Identity and addresses) instead assigns every anonymous member a flat, Frame-scoped ordinal at admission, with no parent reference at all: an ordinal is fixed once assigned and is never recomputed from tree position, so no edit anywhere in the Frame can shift it (RFC-001 §Reconciliation). This also resolves the deeper issue the parent-relative model had: an ordinal tied to one parent could not be shared by a second container, which RFC-001's Worked Exercise (step 8) requires. No further decision is needed here; durable references still require promotion to a name (§Anonymous tolerance above), for a different reason than fragility — see the identity gradient.
