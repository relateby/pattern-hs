...
This RFC proposes a navigation framework for `Pattern Subject` that distinguishes two orthogonal kinds of perspective-shift: **stratum-shifts**, which constructively build new Patterns by embedding selected sub-Patterns as the atomic elements of a higher-order Pattern, and **aspect-shifts**, which interpretively re-view the same Pattern through a different scope. The framework introduces a `Stratum` type — a Pattern Subject wrapped with its own identifying `Subject` and origin references back to what it was based on — along with two small modules, `Pattern.Stratum` and `Pattern.Aspect`, built on the existing `Pattern`, `PatternKind`, `ScopeQuery`, and `RepresentationMap` primitives. Because a `Stratum` carries a `Subject`, strata themselves are addressable; a Pattern whose elements are stratum subjects represents *inter-stratum* topology in the same Pattern/Subject vocabulary as intra-stratum content, with no new types. Strata and aspects compose orthogonally, and the framework's own primitives are expressible in the framework — providing a uniform vocabulary for the graph-of-graphs problem at every scale.
...
**OriginRef** — The provenance record attached to each embedded element of a Stratum: the lower Stratum (when held inline) and the sub-Pattern that was embedded from it.
...
**Embedded from** — An embedded element at a higher Stratum is *embedded from* a sub-Pattern at the lower Stratum. The OriginRef records this.
...
**Promotion** — The mechanism of a stratum-shift: select sub-Patterns of the lower stratum via a scope; embed each as a fresh atom; assemble the embedded elements as the contents of a higher Pattern Subject; wrap the result with the OriginRefs recording what each was embedded from.

**Embedding** *(noun)* — The act of producing one or more embedded elements from selected sub-Patterns of the lower Stratum. Promotion's mechanism.

**Embedded element** — An atomic element of a Stratum that was produced by promotion; the Subject returned by `embed` for some sub-Pattern of the lower Stratum.

**Refinement** — The dual of promotion: navigate from an embedded element at the higher Stratum back to the sub-Pattern it was embedded from, via its OriginRef.
...
### A note on the `embed` metaphor

The function that produces a fresh Subject for each promoted element is named `embed`. The metaphor is webpage embedding: the higher Stratum holds an atomic placeholder element whose Subject names the embedded thing; the actual content lives at its source (the lower Stratum's sub-Pattern); recovery is mediated by the surrounding context (the Stratum's `stratumOrigin` map) rather than by inspecting the placeholder directly. This matches promotion's relationship between embedded elements and their sources at every joint: the embed names without containing, the original is intact at its source, refinement is context-mediated, and ordinary traversal of the higher Stratum stops at the embedded element just as a webpage's DOM traversal stops at an `<iframe>`.
...
1. **Constructive moves** that build a new Pattern by embedding sub-Patterns of an existing one as the atomic elements of a higher-order Pattern — people-Patterns become embedded elements of a neighborhood-Pattern, neighborhood-Patterns become embedded elements of a city-Pattern. These produce new `Subject` identities and new `Pattern` values. They are lossy at the new level in the sense that the source sub-Pattern's internal structure is not visible to ordinary traversal of the result.
...
A **stratum** is a Pattern Subject wrapped as a Stratum — addressable, based on what it was promoted from. Strata stack: one stratum's contents can be promoted to become the embedded elements of the next.
...
data Stratum = Stratum
  ...
  , stratumOrigin  :: Map Symbol OriginRef
    -- ^ For each embedded element (keyed by its Subject identity — `Symbol`,
    --   which is `Id Subject` in the library's `GraphValue` instance), what
    --   lower-stratum sub-Pattern it was embedded from.
  }
...
data OriginRef = OriginRef
  ...
  , originPattern :: Pattern Subject
    -- ^ The lower-stratum sub-Pattern that was embedded. Always a single
...
A promotion is a selector plus an embedder:
...
data Promotion = Promotion
  ...
  , promotionSelect   :: forall q. ScopeQuery q Subject
    ...
    -- ^ Which sub-Patterns of the lower Pattern become embedded elements
  ...
  , embed             :: Pattern Subject -> Subject
    -- ^ Given a selected sub-Pattern, produce a fresh Subject for the
    --   embedded element at the higher stratum. The Subject is the
    --   identity by which the embedded element will be looked up; the
    --   link back to the source sub-Pattern is stored separately in the
    --   produced Stratum's origin map. Required to be injective.
  }
```

The naming choice deserves a sentence. `embed` is the function that produces a placeholder element at the higher stratum standing in for a sub-Pattern at the lower stratum. The metaphor is webpage embedding: the higher Stratum's element-structure contains only the embed (the atomic Subject), not the embedded content; the original content remains intact at its source; recovery passes through the surrounding context (`stratumOrigin`) rather than through the placeholder itself. The metaphor matches the semantics precisely, and the contract — injectivity, lossiness, fresh identity — is what one expects of an embed.
...
**Small-scale connections** are *intra-stratum*: they live inside a single stratum and tie its embedded elements to the lower-stratum sub-Patterns they were embedded from. These are the `stratumOrigin` map's job. They answer "what does this embedded element stand for?" and `refine` follows them.
...
Embedded elements are *down-references*: their `Subject` identifies a higher-stratum entity, and their position in `stratumOrigin` records what lower-stratum sub-Pattern they refer to. The down-reference is non-structural in the traversal sense — it lives in the `Stratum`'s origin map, not in the embedded element's `elements`.

This is the load-bearing property of the design: **embedded elements are leaves in the Pattern's element-structure, so ordinary traversal stops at them**. `Foldable`, `Functor`, `Traversable`, `Comonad`, and `paraWithScope` all walk a Pattern by descending into its `elements`; an embedded element has no elements; descent stops. The structural opacity is achieved not by an active guard but by structural absence — there is no lower-stratum content reachable through the higher Pattern's traversal interface. This is exactly the webpage-embed property: a browser walking a host page's DOM stops at the `<iframe>` element; what's inside is a separate concern, resolved through a separate channel.

Refinement is therefore a deliberate, opt-in operation:

```haskell
refine :: Stratum -> Pattern Subject -> Maybe (Pattern Subject)
-- ^ Given an embedded element and the Stratum it belongs to, retrieve
--   the lower-stratum sub-Pattern it was embedded from, if available.
```
...
1. **Identity scheme for embedded elements.** `embed` produces a fresh `Subject` for each embedded element. Should the library specify any conventions for the embedded identity (e.g., deriving deterministically from the source sub-Pattern's identity so that two independent promotions of the same source yield the same higher-stratum identity), or leave this entirely to the caller? Deterministic embedding enables incremental computation and stratum-stable identity; non-deterministic embedding is simpler and avoids the question of what "same source" means when scopes differ.
...
**Naming the promotion function `mint`, `name`, or `project` rather than `embed`.** Considered at length. `mint` (the prior draft's name as `promotionMint`) accurately conveys fresh-identity production but is neutral on the structural relationship between the produced atom and its source. `name` undersells what the function does (it produces a whole Subject, not just an identity). `project` conveys lossiness correctly but stretches the spatial metaphor across the stratum boundary. `embed` was chosen because the webpage-embedding mental model matches the semantics at every joint: the embed names without containing, the original is intact at its source, recovery is context-mediated, and traversal of the host structure stops at the embed. The mathematical reading of "embedding" (a structure-preserving injection) is not what is meant; the RFC commits to the webpage reading and uses it consistently.
